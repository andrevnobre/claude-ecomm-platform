# Cell Module - Complete E-Commerce Cell
# This module creates a complete isolated cell with all necessary resources
...
resource "aws_vpc" "main" {
  ...
  tags = merge(var.tags, {
    Name = "${var.project_name}-${var.environment}-cell-${var.cell_id}"
  })
}
...
resource "aws_security_group" "elasticache" {
  name_description = "Security group for ElastiCache"
  vpc_id          = aws_vpc.main.id
...
resource "aws_ecs_cluster_capacity_providers" "main" {
  ...
  default_capacity_provider_strategy {
    ...
    weight            = var.environment == "prod" ? 1 : 10
    base              = 0
  }
}
...
resource "aws_rds_cluster_instance" "main" {
  count = var.db_engine_mode == "provisioned" ? var.db_instance_count : 0
...
resource "aws_secretsmanager_secret" "db_password" {
  name = "${var.project_name}-${var.environment}-db-password-${var.cell_id}"
...
resource "aws_elasticache_replication_group" "main" {
  ...
  multi_az_enabled           = var.environment == "prod" && var.cache_num_nodes > 1
...
# RDS Enhanced Monitoring Role (for prod only)
